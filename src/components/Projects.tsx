      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {PROJECTS.map((project) => (
          <a
            ...
            className="block bg-[#EDE3CB] rounded-xl p-6 transition-all duration-[400ms] [transition-timing-function:cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-1.5 hover:shadow-[0_12px_32px_rgba(44,44,42,0.12)]"
          >
            <h3 className="text-[1.15rem] font-bold text-[#2C2C2A] mb-2">
              {project.title}
            </h3>
            ...
            <div className="flex items-center justify-between">
              <div className="flex gap-2 flex-wrap">
                {project.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-[0.7rem] uppercase tracking-[0.05em] text-[#B04A1C] bg-[#B04A1C]/10 px-2.5 py-1 rounded-full"
                  >
                    {tag}
                  </span>
                ))}
